"""
输出模块 — SQLite 统一存储 + CSV 导出 + 统计报告。

所有提取结果、分类结果、验证报告统一写入一个 SQLite 数据库文件，
便于 1500 万级论文的大规模存储、查询和下游对接。
...
数据库表结构：
  ...
  validation_issues  验证问题明细（扁平化，一行一条 issue/warning）
...
CREATE TABLE IF NOT EXISTS varieties (
    ...
    variety_source  TEXT,
    yield_raw_value REAL,
    yield_raw_unit  TEXT,
    yield_standard_value REAL,
    ...
    FOREIGN KEY (paper_id, study_index) REFERENCES studies(paper_id, study_index)
);
...
CREATE INDEX IF NOT EXISTS idx_paper_status_run ON paper_status(run_id);

-- 字段文档表（数据字典，DBeaver 可查询）
...
def init_database(db_path: Path) -> sqlite3.Connection:
    """
    初始化 SQLite 数据库，创建所有表、索引和字段文档。

    返回数据库连接。调用方负责关闭连接。
    """
    db_path.parent.mkdir(parents=True, exist_ok=True)
    conn = sqlite3.connect(str(db_path), check_same_thread=False)
    conn.execute("PRAGMA journal_mode=WAL")  # 并发写入性能更好
...
_SCHEMA_DOCS = [
    ...
    ("varieties", "yield_raw_value", "REAL", "论文中的原始产量数值", 1, "LLM提取"),
    ("varieties", "yield_raw_unit", "TEXT", "原始产量单位，如kg/亩、kg/ha", 1, "LLM提取"),
    ("varieties", "yield_standard_value", "REAL", "程序换算的kg/ha值", 0, "程序计算"),
    ...
    ("varieties", "pct_over_check", "REAL", "相对对照品种的增产/减产百分比", 0, "LLM提取"),
    ...
    ("varieties_flat", "latitude", "REAL", "纬度", 0, "LLM/地理编码"),
    ("varieties_flat", "longitude", "REAL", "经度", 0, "LLM/地理编码"),
    ...
    ("varieties_flat", "yield_raw_value", "REAL", "原始产量数值", 1, "LLM提取"),
    ("varieties_flat", "yield_raw_unit", "TEXT", "原始产量单位", 1, "LLM提取"),
    ("varieties_flat", "yield_standard_value", "REAL", "换算后kg/ha值", 0, "程序计算"),
    ("varieties_flat", "pct_over_check", "REAL", "增产/减产百分比", 0, "LLM提取"),
    ...
    ("classification", "confidence", "REAL", "分类置信度（0-1）", 0, "LLM评估"),
    ...
    ("paper_status", "status", "TEXT", "处理状态: completed/skipped/failed/error", 1, "系统记录"),
    ("paper_status", "duration_sec", "REAL", "处理耗时（秒）", 0, "系统记录"),
...
def _populate_schema_doc(conn: sqlite3.Connection):
    """填充字段文档表（幂等操作，INSERT OR REPLACE）。"""
    for table, col, col_type, desc, required, source in _SCHEMA_DOCS:
        conn.execute("""
            INSERT OR REPLACE INTO _schema_doc
            (table_name, column_name, column_type, description, is_required, source)
            VALUES (?, ?, ?, ?, ?, ?)
...
def insert_extraction(conn: sqlite3.Connection, result: dict, paper_id: str):
    """
    将一篇论文的提取结果写入数据库（papers + studies + varieties 三表）。
    ...
    for si, study in enumerate(studies):
        ...
        """, (
            ...
            study.get("sowing_date"),
            study.get("harvest_date"),
            study.get("country"),
            ...
            study.get("longitude"),
            study.get("altitude"),
            study.get("geo_source"),
            study.get("replication_number"),
            study.get("plot_size"),
            study.get("planting_density"),
            study.get("experimental_design_description"),
            study.get("experimental_design_type"),
            study.get("growth_facility_description"),
            study.get("cultural_practices"),
            study.get("notes"),
        ))

        # ── varieties 表 + varieties_flat 宽表 ──
        ...
        for vi, v in enumerate(varieties):
            ...
            """, (
                ...
                v.get("variety_code"),
                is_ck_int,
                v.get("variety_source"),
                v.get("yield_raw_value"),
                v.get("yield_raw_unit"),
                v.get("yield_standard_value"),
                v.get("yield_standard_unit", "kg/ha"),
                v.get("yield_value_type"),
                v.get("significance_group"),
                v.get("pct_over_check"),
                v.get("measurement_method"),
                v.get("source_location"),
                v.get("confidence_level"),
            ))

            # 宽表：paper + study + variety 合并为一行（交接用）
            conn.execute("""
                INSERT OR REPLACE INTO varieties_flat
...
def insert_classification(conn: sqlite3.Connection, records: List[dict]):
    """批量写入分类结果。"""
    if not records:
        return

    for cls in records:
        # key_signals 列表转 JSON 字符串存储
        ...
        """, (
            ...
            cls.get("reasoning"),
            key_signals,
            cls.get("crop_species"),
            cls.get("paper_type"),
            has_yield_int,
            cls.get("research_country"),
        ))
...
def insert_validation(conn: sqlite3.Connection, results: List[dict]):
    """
    将验证报告扁平化写入 validation_issues 表。

    每篇论文的 issues 和 warnings 各展开为独立行。
    """
    if not results:
        return
    ...
    for record in results:
        ...
        for issue in report.get("issues", []):
            conn.execute("""
                INSERT INTO validation_issues (paper_id, issue_type, severity, message)
                VALUES (?, ?, ?, ?)
...
def update_paper_status(
    ...
    conn.execute("""
        INSERT OR REPLACE INTO paper_status
...
def export_table_csv(conn: sqlite3.Connection, table_name: str, csv_path: Path):
    """将指定表导出为 CSV 文件。"""
...
def export_delivery_csv(conn: sqlite3.Connection, csv_path: Path):
    """
    导出交接用宽表 CSV（varieties_flat）。

    这是给下游单位的核心交付物，每行包含 paper+study+variety 全部字段。
    """
...
def get_table_stats(conn: sqlite3.Connection) -> dict:
    """获取各表的行数统计。"""
    stats = {}
    for table in ("papers", "studies", "varieties", "varieties_flat", "classification", "validation_issues", "paper_status"):
